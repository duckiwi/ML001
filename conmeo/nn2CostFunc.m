function [J, grad] = nn2CostFunc(nn_params, layer_size, X, y, lambda)
%%  Implements the neural network cost function for a two layer neural network which performs multi-class classification
...
% ff to calculate all hypothesis. store values for each sample in a row.
a1_all = [ ones(m, 1), X];
...
% first term corresponding to non regularized cost
s = 0;
for cnt = 1:m
